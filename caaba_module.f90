! Time-stamp: <2020-08-13 14:10:19 sander>
...
CONTAINS
  ...
  SUBROUTINE caaba_read_nml(status, iou)
    ...
    INTEGER, PARAMETER :: MAX_SCENARIOS = 23
    ...
    LOGICAL :: l_aqueous_scenario_ok   = .FALSE.
    ...
    DO i=1, MAX_SCENARIOS
      ...
      IF (TRIM(list_of_scenarios(i))==TRIM(aqueous_scenario)) &
        l_aqueous_scenario_ok = .TRUE.
    ENDDO
    ...
    ELSE
      ...
      STOP 1
    ENDIF
    IF (l_aqueous_scenario_ok) THEN
      WRITE(*,*) '  Aqueous: ', TRIM(aqueous_scenario)
    ELSE
      WRITE(*,*) 'ERROR: unknown aqueous scenario ', TRIM(aqueous_scenario)
      STOP 1
    ENDIF
    WRITE(*,*) HLINE2
  ...
  SUBROUTINE caaba_physc
    ! USE caaba_mem,                ONLY: model_time, temp, press, relhum, cair
      USE caaba_mem,                ONLY: model_time, zmbl, zmbl_array
    ...
    !
    !Converting model_time(s) to mod_tim_hr(integer hours)
       mod_tim_hr = model_time/3600.
    !zmbl is assigned value from zmbl_array depending on the value
    !value of the model time(truncated to nearest hour)
!       zmbl for IITD
       zmbl = zmbl_array(mod((mod_tim_hr+5.5), 24.))
     !zmbl for UK, Leicester
!       zmbl = zmbl_array(mod((mod_tim_hr + 0.0), 24.))
