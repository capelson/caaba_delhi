!*****************************************************************************
! Time-stamp: <2022-04-21 19:40:23 sander>
!*****************************************************************************
...
#define c C_           /* renamed into something else, e.g. C_(:)           */
  ...
  USE messy_mecca_kpp           ! DP, ind_*, kpp_integrate, rconst,
                                ...
                                ! SPC_NAMES, EQN_TAGS, NSPEC, NREACT, inifile
#ifdef MECCA_TAG
...
CONTAINS
  ...
  SUBROUTINE mecca_init
    ...
    USE messy_mecca,              ONLY: mecca_read_nml_ctrl, define_mcexp
  ...
  SUBROUTINE mecca_aero_init_gasaq()
    ...
    USE messy_main_constants_chemprop_mem ! chemprop, get_chemprop_index, R_*, ...
    ...
    PRINT *, HLINE2
    PRINT *, "         Henry's law coefficients and accommodation coefficients"
    PRINT *, HLINE2
  ...
  SUBROUTINE define_aerosol
    ...
    USE caaba_mem, ONLY: degree_lat, zmix, zmbl, aqueous_scenario
    IMPLICIT NONE
...
!         c0_HSO4m(1) = csalt(1)
!         exchng(1)   = 0.
!       CASE ('CLOUD','CLOUDEVENT')
!         ! index 1 = sulfate aerosol
!         xaer(1)     = 1.
!         radius(1)   = 20.*1.E-6 ! radius [m]
!         lwc(1)      = 3.*1.E-7 ! g/m3 -> m3/m3 
!         csalt(1)    = 0.
!         c0_NH4p(1)  = csalt(1)
!         c0_HCO3m(1) = 0.
!         c0_NO3m(1)  = 0.75*csalt(1)
!         c0_Clm(1)   = 0.
!         c0_Brm(1)   = 0.
!         c0_Im(1)    = 0.
!         c0_IO3m(1)  = 0.
!         c0_SO4mm(1) = 0.2*csalt(1)
!         c0_HSO4m(1) = 0.05*csalt(1)
!         exchng(1)   = 1. / (7.*OneDay) ! exchange with fresh aerosol [1/s]
!       CASE DEFAULT
!         PRINT *, 'ERROR in define_aerosol: aqueous_scenario "'// &
!           TRIM(aqueous_scenario)//'" is not defined'
!         STOP 1
!       END SELECT
  ...
  SUBROUTINE x0
    ...
    USE caaba_mem,                ONLY: init_scenario, time0_jul
  ...
  CONTAINS
    ...
    SUBROUTINE x0_simple
      !Initial values for Leicester, UK (Sommariva et al.)
      IF (ind_O2       /= 0) c(ind_O2)      = 210.E-03 * cair
      IF (ind_N2       /= 0) c(ind_N2)      = 780.E-03 * cair
      IF (ind_CH4      /= 0) c(ind_CH4)     =  1.9E-06 * cair
      IF (ind_H2O       /= 0) c(ind_H2O)      = 0.0055 * cair
      IF (ind_CO       /= 0) c(ind_CO)      = 140.E-09 * cair
      IF (ind_O3       /= 0) c(ind_O3)      =  20.E-09 * cair
      IF (ind_NO2       /= 0) c(ind_NO2)      =  19.E-09 * cair
      IF (ind_HONO     /= 0) c(ind_HONO)    =  0.13E-09 * cair
      IF (ind_C5H8     /= 0) c(ind_C5H8)    = 0.02E-09 * cair
      IF (ind_C2H6     /= 0) c(ind_C2H6)    = 6.3E-09 * cair
      IF (ind_C2H4     /= 0) c(ind_C2H4)    = 0.67E-09 * cair
      IF (ind_C2H2     /= 0) c(ind_C2H2)    = 0.49E-09 * cair
      IF (ind_C3H8     /= 0) c(ind_C3H8)    = 2.30E-09 * cair
      IF (ind_C3H6     /= 0) c(ind_C3H6)    = 0.19E-09 * cair
      IF (ind_IC4H10   /= 0) c(ind_IC4H10)  = 0.59E-09 * cair
      IF (ind_NC4H10   /= 0) c(ind_NC4H10)  = 0.97E-09 * cair
      IF (ind_BUT1ENE  /= 0) c(ind_BUT1ENE) = 0.05E-09 * cair
      IF (ind_CBUT2ENE  /= 0) c(ind_CBUT2ENE) = 0.02E-09 * cair
      IF (ind_TBUT2ENE  /= 0) c(ind_TBUT2ENE) = 0.02E-09 * cair
      IF (ind_BENZENE  /= 0) c(ind_BENZENE) = 0.20E-09 * cair
      IF (ind_TOLUENE  /= 0) c(ind_TOLUENE) = 0.19E-09 * cair
      IF (ind_EBENZ    /= 0) c(ind_EBENZ) = 0.03E-09 * cair
      IF (ind_LXYL  /= 0) c(ind_LXYL) = 0.16E-09 * cair
      IF (ind_LTMB  /= 0) c(ind_LTMB) = 0.10E-09 * cair
      IF (ind_ClNO2 /= 0) c(ind_ClNO2)= 0.31E-09 * cair
    ...
    SUBROUTINE x0_mom
      ...
      IF ((inifile=="mcm").OR.(inifile=="jam")) THEN
    ...
    SUBROUTINE x0_cloud
      ...
      ! adjusted data from Tab. 2 of Taraborrelli et al., ACP, 9, 2751-2777 (2009):
      IF (ind_H2O2     /= 0) c(ind_H2O2)     =   7.E-09 * cair
      IF (ind_O3       /= 0) c(ind_O3)       =  30.E-09 * cair
      IF (ind_O2       /= 0) c(ind_O2)       = 210.E-03 * cair
      IF (ind_NH3      /= 0) c(ind_NH3)      = 100.E-12 * cair
      ...
      IF (ind_ACETOL   /= 0) c(ind_ACETOL)   =  4.0E-09 * cair
      IF (ind_MGLYOX   /= 0) c(ind_MGLYOX)   = 500.E-12 * cair
      IF (ind_C5H8     /= 0) c(ind_C5H8)     = 100.E-12 * cair
      IF (ind_PAN      /= 0) c(ind_PAN)      = 100.E-12 * cair
      IF (ind_C2H6     /= 0) c(ind_C2H6)     =   2.E-09 * cair

    END SUBROUTINE x0_cloud
...
!!#D radjimt -
    ...
    USE caaba_mem,               ONLY: l_skipkpp, photol_clev, zmix, &
                                       l_ignore_relhum, lhour, aqueous_scenario
    ...
    ! from Simon Rosanka, 2020:
    IF (TRIM(aqueous_scenario) == 'CLOUDEVENT') THEN
      IF ((lhour >= 12.).AND.(lhour < 13.)) THEN
        xaer(1) = 1.
      ELSE
        CALL cloud_evap
        xaer(1) = 0.
...
!!#D radjimt -
    ...
    IF (l_aero) THEN
      ...
      CALL mecca_aero_calc_k_ex( &
        ...
        k_exf, k_exb, k_exf_N2O5, k_exf_ClNO3,k_exf_ClNO2, k_exf_BrNO3) ! out
      ...
      CALL fill_k_exf_ClNO2(status, SPREAD(k_exf_ClNO2,1,NBL))
  ...
  CONTAINS
    ...
    SUBROUTINE aerosol_exchng
      ...
      !REAL :: tau_aerosol = 7200. ! chamber lifetime  2700 (45) --> 7200 (120)
...
!!$        !----------
    ...
    SUBROUTINE cloud_evap ! from Simon Rosanka, 2020
      ...
      DO jb = 1,APN
        DO i = 1,NSPEC
          ...
          IF (INDEX(SPC_NAMES(i),'_a'//TRIM(str(jb,'(I2.2)'))) /= 0) THEN
            IF (INDEX(SPC_NAMES(i),'RR') /= 1) THEN
              DO j = 1,NSPEC
                IF (TRIM(SPC_NAMES(j))//'_a'//TRIM(str(jb,'(I2.2)')) == SPC_NAMES(i)) THEN
                  c(j) = c(i) + c(j)
                  EXIT
                ENDIF
              ENDDO
  ...
  SUBROUTINE mecca_finish
    ...
    ! op_pj_20160218: When a vectorized solver is used, rconst has rank
    ! 2 but the first dimension length is 1 (only one box in CAABA).
    ! PACK reduces the rank of rconst back to 1:
    CALL write_output_file(ncid_k_end, model_time, PACK(rconst,.TRUE.))
    CALL close_file(ncid_k_end)

    ! qqq+ this section is only for ferret; it will be obsolete soon
    IF (REQ_MCFCT) THEN
